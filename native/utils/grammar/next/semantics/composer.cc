...
#include "utils/grammar/next/semantics/composer.h"

#include "utils/base/status_macros.h"
#include "utils/grammar/next/semantics/evaluators/compose-eval.h"
#include "utils/grammar/next/semantics/evaluators/const-eval.h"
#include "utils/grammar/next/semantics/evaluators/constituent-eval.h"
#include "utils/grammar/next/semantics/evaluators/merge-values-eval.h"
#include "utils/grammar/next/semantics/evaluators/parse-number-eval.h"
#include "utils/grammar/next/semantics/evaluators/span-eval.h"
...
StatusOr<const SemanticValue*> SemanticComposer::Eval(
  ...
  for (const auto& [constituent_index, constituent] :
    ...
    if (constituent->type == ParseTree::Type::kExpression) {
      ...
      // defined.
      context.rule_constituents[constituent_index] =
          arena->AllocAndInit<SemanticValue>(
              text_context.Span(constituent->codepoint_span));
    }
  }
