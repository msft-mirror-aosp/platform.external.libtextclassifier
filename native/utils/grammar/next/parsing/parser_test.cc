...
#include "utils/grammar/next/parsing/parser.h"

#include <string>
#include <vector>

#include "utils/grammar/next/parsing/derivation.h"
#include "utils/grammar/next/testing/utils.h"
...
TEST_F(ParserTest, ParsesSimpleRules) {
  ...
  EXPECT_THAT(parser.Parse(TextContextForText("Event: 2020/05/08"), &arena_),
              ElementsAre(IsDerivation(kDate, 7, 17)));
}

TEST_F(ParserTest, HandlesUppercaseTokens) {
  Rules rules;
  const int kScriptedReply = 0;
  ...
  EXPECT_THAT(
      parser.Parse(TextContextForText("Reply STOP to cancel."), &arena_),
...
TEST_F(ParserTest, HandlesAnchors) {
  ...
  EXPECT_THAT(parser.Parse(TextContextForText("Reply STOP"), &arena_),
              ElementsAre(IsDerivation(kScriptedReply, 0, 10)));

  EXPECT_THAT(
      parser.Parse(TextContextForText("Please reply STOP to cancel."), &arena_),
      IsEmpty());
}
...
TEST_F(ParserTest, HandlesWordBreaks) {
  ...
  EXPECT_THAT(
      parser.Parse(TextContextForText("My flight is: LX 38. Arriving later"),
                   &arena_),
      ElementsAre(IsDerivation(kFlight, 14, 19)));

  // Make sure the grammar doesn't trigger on "LX 38.00".
  EXPECT_THAT(parser.Parse(TextContextForText("LX 38.00"), &arena_), IsEmpty());
}

TEST_F(ParserTest, HandlesAnnotations) {
  Rules rules;
  const int kCallPhone = 0;
  ...
  // annotations.
  EXPECT_THAT(parser.Parse(context, &arena_), IsEmpty());
  ...
  EXPECT_THAT(parser.Parse(context, &arena_),
...
TEST_F(ParserTest, HandlesExclusions) {
  ...
  EXPECT_THAT(
      parser.Parse(TextContextForText("do not forget to be there"), &arena_),
      ElementsAre(IsDerivation(kSetReminder, 0, 25)));

  EXPECT_THAT(
      parser.Parse(TextContextForText("do not forget to be safe"), &arena_),
      IsEmpty());
}
...
TEST_F(ParserTest, HandlesWhitespaceGapLimit) {
  ...
  EXPECT_THAT(parser.Parse(TextContextForText("LX38 aa 44 LX 38"), &arena_),
